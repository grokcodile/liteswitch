...
- **Pause in Apps** (in the ⓘ popover) lists apps that should have the keyboard to themselves — a remote session, a game, anything that owns the same chords. Liteswitch unregisters its shortcuts while one of them is frontmost and takes them back on the way out, so there is nothing to shadow the app's own bindings.
