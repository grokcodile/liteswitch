...
- **Dictate Text** — hold **Right ⌥** or **Right ⌘** and it dictates; release and it stops. That is the whole of it: the same thing the dictation key does, on a key you can hold, with no cleanup pass afterwards. A waveform meter is up while it listens, and stays up for about two seconds after you let go while macOS finishes transcribing — press the key again in that window and it carries straight on. To tidy up what you said, select it and run **Clean Dictation** from Rewrite Text.
